...
2. **패치 적용** — 최신 패치 `SDGundamGashaponWars_KR_v2.5.xdelta`
   - Windows: [Delta Patcher](https://github.com/marco-calautti/DeltaPatcher/releases) 로 원본 ISO + 패치 선택 → Apply
   - 또는 커맨드라인: `xdelta3 -d -s "SD Gundam Gashapon Wars.iso" SDGundamGashaponWars_KR_v2.5.xdelta "SD Gundam Gashapon Wars (KR).iso"`
3. **결과 확인** — v2.5 패치 후 ISO CRC32: `E5007024`
...
- 🔤 **본문 폰트를 나눔스퀘어 네오 Bold로 교체** — 가독성 향상, 우측 클립 수정
  - ※ 대형 메뉴 타이틀 이미지(bank102 368×74)는 v2.1에서 시도했으나 인게임 표시 문제로 v2.5에서 원복했습니다.
